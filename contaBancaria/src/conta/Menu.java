...
import conta.bancaria.model.ContaCorrente;
import conta.bancaria.model.ContaPoupanca;
...
public class Menu {
	...
	// Objeto controlador de contas (responsável por cadastrar, listar, atualizar
	// etc.)
	...
	public static void main(String[] args) {
		...
		// Cria algumas contas de teste para o sistema não começar com a Collection
		// vazia
		criarContasTeste();
	...
	// Método responsável por criar algumas contas de teste automaticamente ao
	// iniciar o programa
	...
	// Método responsável por criar uma nova conta com os dados digitados pelo
	// usuário
	...
	private static void atualizarConta() {
		...
		if (conta != null) {
			...
			System.out.printf(
					"Agência atual: %d\nDigite o número da nova Agência (Pressione ENTER para manter o valor atual): ",
					agencia);
			 ...
			 * inteiro e atribui à variável 'agencia'.
			 * 
			 * Operador Ternário: condição ? ação caso seja verdadeira : ação caso seja
			 * falsa
			 */
			...
			System.out.printf(
					"Titular atual: %s\nDigite o novo nome do Titular (Pressione ENTER para manter o valor atual): ",
					titular);
			entrada = leia.nextLine();
			titular = entrada.isEmpty() ? titular : entrada;

			// Atualiza saldo (ou mantém valor atual se apertar Enter)
			System.out.printf("Saldo atual: %.2f\nDigite o novo Saldo (Pressione ENTER para manter o valor atual): ",
					saldo);
			entrada = leia.nextLine();
			...
			case 1 -> {
				...
				/**
				 * Como o objeto 'conta' é do tipo genérico Conta, precisamos convertê-la
				 * (casting) para ContaCorrente. Isso é necessário porque apenas a classe
				 * ContaCorrente possui o atributo 'limite'. Após o casting, conseguimos acessar
				 * o método getLimite() para obter o limite da conta.
				 */
				float limite = ((ContaCorrente) conta).getLimite();

				// Atualiza o limite da conta (ou mantém valor atual se apertar Enter)
				System.out.printf(
						"Limite atual é: %.2f\nDigite o novo Limite (Pressione ENTER para manter o valor atual): ",
						limite);
				entrada = leia.nextLine();
				 ...
				 * 
				 * Se chamarmos 'gerarNumero()', um novo número seria criado e substituiria o
			...
			case 2 -> {
				...
				/**
				 * Como o objeto 'conta' é do tipo genérico Conta, precisamos convertê-la
				 * (casting) para ContaPoupanca. Isso é necessário porque apenas a classe
				 * ContaPoupanca possui o atributo 'aniversario'. Após o casting, conseguimos
				 * acessar o método getAniversario() para obter o dia do aniversário da conta.
				 */
